...
mod tests {
    ...
    #[test]
    fn bare_dollar_sign() {
        let result = substitute("cost: $5", &vars(&[])).unwrap();
        assert_eq!(result, "cost: $5");
    }

    #[test]
    fn double_dollar_escape() {
        let result = substitute("$$VAR", &vars(&[("VAR", "hello")])).unwrap();
        assert_eq!(result, "$VAR");
    }

    #[test]
    fn bare_var_set() {
        let result = substitute("$FOO bar", &vars(&[("FOO", "hello")])).unwrap();
        assert_eq!(result, "hello bar");
    }

    #[test]
    fn bare_var_unset() {
        let result = substitute("$MISSING", &vars(&[])).unwrap();
        assert_eq!(result, "");
    }

    #[test]
    fn braced_var_set() {
        let result = substitute("${FOO}", &vars(&[("FOO", "world")])).unwrap();
    ...
    #[test]
    fn default_if_unset_or_empty_set_nonempty() {
        let result = substitute("${FOO:-def}", &vars(&[("FOO", "bar")])).unwrap();
        assert_eq!(result, "bar");
    }

    #[test]
    fn default_if_unset_or_empty_set_empty() {
        let result = substitute("${FOO:-def}", &vars(&[("FOO", "")])).unwrap();
        assert_eq!(result, "def");
    }

    #[test]
    fn default_if_unset_or_empty_unset() {
        let result = substitute("${FOO:-def}", &vars(&[])).unwrap();
        assert_eq!(result, "def");
    }

    #[test]
    fn default_if_unset_set_empty() {
        // `${VAR-default}` — empty value is OK, return empty
        let result = substitute("${FOO-def}", &vars(&[("FOO", "")])).unwrap();
        assert_eq!(result, "");
    }
    ...
    #[test]
    fn alt_if_set_and_nonempty_set_nonempty() {
        let result = substitute("${FOO:+alt}", &vars(&[("FOO", "bar")])).unwrap();
        assert_eq!(result, "alt");
    }

    #[test]
    fn alt_if_set_and_nonempty_set_empty() {
        let result = substitute("${FOO:+alt}", &vars(&[("FOO", "")])).unwrap();
        assert_eq!(result, "");
    }

    #[test]
    fn alt_if_set_and_nonempty_unset() {
        let result = substitute("${FOO:+alt}", &vars(&[])).unwrap();
        assert_eq!(result, "");
    }

    #[test]
    fn alt_if_set_set_empty() {
        // `${VAR+value}` — empty value still counts as "set"
        let result = substitute("${FOO+alt}", &vars(&[("FOO", "")])).unwrap();
        assert_eq!(result, "alt");
    }
    ...
    #[test]
    fn error_if_unset_or_empty_set_nonempty() {
        let result = substitute("${FOO:?err}", &vars(&[("FOO", "bar")])).unwrap();
        assert_eq!(result, "bar");
    }
    ...
    fn error_if_unset_or_empty_unset() {
        ...
        assert!(matches!(result, Err(ComposeError::RequiredVarNotSet { var, msg }) if var == "FOO" && msg == "err msg"));
    }

    #[test]
    fn error_if_unset_or_empty_empty() {
        let result = substitute("${FOO:?err msg}", &vars(&[("FOO", "")]));
        assert!(result.is_err());
    }

    #[test]
    fn error_if_unset_set_empty() {
        // `${VAR?error}` — empty value is OK (only unset triggers error)
        let result = substitute("${FOO?err}", &vars(&[("FOO", "")])).unwrap();
        assert_eq!(result, "");
    }

    #[test]
    fn error_if_unset_unset() {
        let result = substitute("${FOO?err}", &vars(&[]));
        assert!(result.is_err());
    }
    ...
    fn chained_substitutions() {
        ...
        let result = substitute("$A ${B}", &v).unwrap();
        assert_eq!(result, "hello world");
    }

    #[test]
    fn substitution_in_yaml_string() {
        let yaml = "image: myapp:${TAG:-latest}";
        let v = vars(&[]);
        let result = substitute(yaml, &v).unwrap();
        assert_eq!(result, "image: myapp:latest");
    }
    ...
    fn process_env_takes_precedence() {
        ...
        let env_path = dir.path().join(".env");
        let mut f = std::fs::File::create(&env_path).unwrap();
        // Use a var that is certainly set in the process environment.
        writeln!(f, "PATH=/should/not/override").unwrap();

        let map = load_dotenv(dir.path());
        // PATH should NOT be in map because it's in process env.
